...
4. Eject the SD card after writing, insert the card into your board and power on - the bootloader flashes automatically
5. A successful flash deletes `firmware.bin` from the card

**If it doesn't flash, try a different card**. This board is picky about what cards it will and won't flash firmware files from. When flashing, the LED on the Aquila board blinks quickly; you can also see serial output if you're connected to the board via serial.
...
*grblHAL is licensed under GPLv3. The HC32 SDK files located in the `SDK/` directory are subject to the Huada Semiconductor (HDSC) license.*